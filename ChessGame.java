...
public class ChessGame extends JFrame {
    ...
    private void updateDifficulty() {
        if (!gameStarted) {
            ...
            statusLabel.setText(
                    "Difficulty set to: " + difficultyCombo.getSelectedItem() + " - Click 'Start Game' to begin");
        }
    }
    ...
    /**
     * Shared logic for executing a player move, checking game-end, and triggering
     * AI.
     */
    private void executePlayerMove(Move move) {
        ...
        javax.swing.Timer timer = new javax.swing.Timer(500, e -> makeComputerMove());
        timer.setRepeats(false);
        timer.start();
    }
    ...
    private void showPossibleMoves(int fromRow, int fromCol) {
        ...
        // Generate all valid moves for the piece from getAllValidMoves and display them
        List<Move> allMoves = board.getAllValidMoves(selectedPiece.getColor());
        ...
        for (Move move : allMoves) {
            ...
            // Skip duplicate promotion moves (only show one dot per promotion square)
            if (isPawnPromotion && move.promotion != null && move.promotion != PieceType.QUEEN)
                continue;

            boolean isCastling = selectedPiece.getType() == PieceType.KING &&
                    Math.abs(toCol - fromCol) == 2;

            if (isCastling) {
                highlightSquare(toRow, toCol, new Color(100, 150, 255));
                ...
                squares[toRow][toCol].setText("\u25CF" + targetPiece.getSymbol()); // ● + piece
            } else {
